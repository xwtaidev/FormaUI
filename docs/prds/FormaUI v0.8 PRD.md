...
## 12. v0.8.6 / v0.8.7 增补范围（已执行）

> 说明：以下为 v0.8.5 GA 之后的同版本线增补工作记录，用于补充 PRD 与执行计划的一致性追踪。

### 12.1 v0.8.6 - 文档体验与结构治理增强

#### 目标

1. 提升文档站信息密度与导航可用性，降低长文档阅读与章节跳转成本。  
2. 对齐 FormaUI 品牌特征，完成 Fumadocs 主题接入后的视觉修整。  
3. 清理历史遗留页面入口，进一步收敛到 `content/docs` 为核心内容源。

#### 交付内容

1. 侧边栏体验优化：顶部品牌区（项目名 + logo）、章节分组、底部 GitHub 导航入口。  
2. 目录交互修复：下拉默认折叠、长侧栏可滚动、展开点击与主题切换交互修复。  
3. 样式体系修整：  
   - 接入并强化主题色（含暗色模式主色特征）  
   - 优化滚动条出现/消失导致的布局漂移  
   - 调整文档整体字号与标题区布局（含 Copy Markdown 操作区）  
4. 文档内容治理：  
   - 清理 `apps/docs/app` 下未使用 MDX 与 `landing` 目录  
   - Foundations / Components 增加总览页并与目录分组入口关联  
   - Components 文档按文件夹维度重组（降低目录噪声）

#### 验收补充

1. 侧边栏分组、滚动、折叠与主题切换行为稳定。  
2. 暗色模式在保持默认背景的前提下具备品牌色识别。  
3. 文档结构以 `content/docs` 为主，无冗余旧入口干扰主要导航。

### 12.2 v0.8.7 - 组件文档标准化升级

#### 目标

1. 将基础组件文档统一升级到开源项目可维护标准。  
2. 建立“可视化展示 + 代码示例 + API Reference + 可访问性”统一模板。  
3. 提升示例可操作性与一致性，降低使用者迁移成本。

#### 交付内容

1. 核心文档重写（中文）并统一结构：  
   - Button（新增交互展示区与案例结构）  
   - Introduction / Installation / Quick Start（内容深度提升）  
2. 基础组件批量标准化：  
   - Typography、LibCn、Label、Input、Textarea、Card  
   - Badge、Separator、Skeleton、Image  
3. Demo 资产沉淀：新增独立 demo 组件文件，避免 MDX 内联复杂逻辑导致的不稳定。  
4. 代码展示体验增强：  
   - 代码块语言与标题展示统一  
   - 组件案例采用“上方 demo + 下方示例代码”双区块模式

#### 验收补充

1. 以上组件页均具备：可视化展示、安装方式、使用方式、案例、API Reference、可访问性说明。  
2. 文档构建链路稳定（`pnpm --filter docs build` 持续通过）。  
3. 组件文档信息架构与写作风格对齐统一模板，便于后续规模化扩展。

### 12.3 证据提交（参考）

1. `dfee33b` / `b9ea740` / `cf9294f` / `d26e208` / `d1bc3b7`（导航与主题体验增强）  
2. `3893162` / `9662159` / `8a4fb67` / `e6e60ac` / `dd41e1e`（结构治理与总览页建设）  
3. `2f2f50d` / `127c828` / `dd37f36`（组件文档标准化与 demo 体系）
